...
export default function ReportsScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const baseRequests = Promise.all([
        ...
        getExpenses(token),
        getSalesByItem(token, activePeriod, { from: fromStr, to: toStr, branch_id: selectedBranch?.id }),
        getSalesByPaymentType(token, activePeriod, { from: fromStr, to: toStr, branch_id: selectedBranch?.id }),
      ]);
      ...
      const sliceRequests =
        sliceKeys.length > 0
          ? Promise.all(
              sliceKeys.map((s) =>
                getSalesSummary(token, 'custom', { from: s.from, to: s.to, branch_id: selectedBranch?.id }),
              ),
            )
  ...
  const timeSeriesChartData = dailySummaries.map(({ date: label, netSales }, i) => ({
    ...
    label:
      period === 'this_month'
        ? i % monthlyInterval === 0 ? label : ''
        : period === 'custom' && customGranularity === 'day'
          ? i % customDayInterval === 0 ? label : ''
          : label,
    frontColor: '#3B55D5',
  }));
  ...
  const chartData = [
    ...
    { value: summary?.netSales ?? 0, label: 'Net Sales', frontColor: '#6C7FE8' },
  ...
  const legendItems = [
    ...
    { label: 'Cost of Goods', value: summary?.costOfGoods ?? 0, color: '#F59E0B' },
    { label: 'Gross Profit', value: grossProfit, color: '#22C55E' },
    { label: 'Expenses', value: -totalExpenses, color: '#EF4444' },
    { label: 'Net Profit', value: netProfit, color: netProfit >= 0 ? '#0EA5E9' : '#EF4444' },
  ];
      ...
      <View className="bg-primary px-5 pb-5 pt-14 dark:bg-primary-700">
        <View className="flex-row items-center">
          ...
          <View className="flex-1 items-center">
            ...
            {selectedBranch ? (
              <Text className="mt-0.5 text-xs font-semibold text-primary-100">
                {selectedBranch.name}
              </Text>
            ) : (
              <Text className="mt-0.5 text-xs text-primary-200">All Branches</Text>
            )}
          </View>
          <TouchableOpacity
        ...
        <View className="gap-4 px-5 pb-10 pt-4">
          {!isSingleDay && (
            <Card title="Sales Overview">
              <View onLayout={(e) => setChartContainerWidth(e.nativeEvent.layout.width)}>
                {isTimeSeries ? (
                  <BarChart
              ...
              <View key={item.label}>
                <View className="flex-row items-center justify-between py-2.5">
                  <View className="flex-row items-center gap-2">
                    ...
                    <Text className="text-sm text-subtle dark:text-muted">{item.label}</Text>
                  </View>
                  <Text className="text-sm font-bold" style={{ color: item.color }}>
                    {item.value < 0 ? `-${fmtPeso(Math.abs(item.value))}` : fmtPeso(item.value)}
                  </Text>
                </View>
