...
import { type Order, type Store } from '@/lib/api';
...
export const OrderReceiptCard = forwardRef<View, Props>(function OrderReceiptCard({ order, store }, ref) {
  const items = order.items ?? [];

  const branchAddress = order.branch?.address ?? store?.address ?? null;
  const contactNumber = store?.phone ?? order.branch?.phone ?? null;
  ...
  const showGcash = isUnpaid && !!store?.gcash_number;
    ...
    <View ref={ref} collapsable={false} className="w-[624px] bg-zinc-100 p-8">
      <View className="overflow-hidden rounded-2xl bg-white">
        ...
        <View className="items-center border-b border-zinc-100 bg-white px-8 pb-10 pt-11">
          ...
          {!!store?.email && <Text className="mt-1 text-xs text-zinc-500">{store.email}</Text>}
        </View>
        ...
        <View className="p-8">
          ...
          {showGcash && (
            <View className="mt-6 items-center border-t border-zinc-100 pt-6">
              ...
              <Text className="mt-2 text-base font-bold text-zinc-900">{store!.gcash_number}</Text>
            </View>
          )}
        </View>
      </View>
